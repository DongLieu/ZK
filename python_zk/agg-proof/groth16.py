...
def keygen(qap: QAP, verbose=False):  # -> (ProverKey, VerifierKey)
    ...
    alpha = FP(7)#FP(random.randint(2, p - 1))
    beta = FP(6)#FP(random.randint(2, p - 1))
    gamma = FP(5)#FP(random.randint(2, p - 1))
    delta = FP(4)#FP(random.randint(2, p - 1))
    tau = FP(1)#FP(random.randint(2, p - 1))
    l= 2#random.randrange(2, qap.L.shape[0])
...
def prove(pk: ProverKey, w: [], qap: QAP, verbose=False):
    r = FP(2)#FP(random.randint(2, p - 1))
    s = FP(3)#FP(random.randint(2, p - 1))
