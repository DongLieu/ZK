...
# aG1 + bG2 + cG3 + dG4
def vector_commit(scalars, points):
...
def commit(a, sL, b, sR, alpha, beta, gamma, tau_1, tau_2):
    A = add(add(vector_commit(a, G), vector_commit(b, H)),multiply(B, alpha))
    S = add(add(vector_commit(sL, G), vector_commit(sR, H)) , multiply( B, beta))
    V = add(vector_commit(a*b, G) , multiply( B, gamma))
    T1 = add(vector_commit(a * sR + b*sL, G) , multiply(B, tau_1))
    T2 =add( vector_commit(sL * sR, G) , multiply( B, tau_2))
    return (A, S, V, T1, T2)
...
sL = np.array([1,8,4,82])
sR = np.array([12,18,42,2])
...
_G =vector_commit(np.array([1,1,1,1]),G)
print(_G)
print("t_u==",t_u)
_MG = multiply(_G, t_u)
print(_MG)
assert t_u == np.mod(np.inner(np.array(l_u), np.array(r_u)), p), "tu !=〈lu, ru〉"
assert eq(add(A, multiply(S, u)), add_points(vector_commit(l_u,G), vector_commit(r_u,H), multiply(B, pi_lr))), "l_u or r_u not evaluated correctly"
assert eq(add(_MG, multiply(B, pi_t)), add_points(V, multiply(T1, u), multiply(T2, u**2 % p))), "t_u not evaluated correctly"
