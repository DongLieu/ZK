...
def commit(a, sL, b, sR, alpha, beta, gamma, tau_1, tau_2):
    A = add(vector_commit(G, a), vector_commit(H, sL), multiply(B, alpha))
    S = add(vector_commit(G, b), vector_commit(H, sR), multiply(B, beta))
    V = add(vector_commit(G, a * b), multiply(B, gamma))
    T1 = multiply(B, tau_1)
    T2 = multiply(B, tau_2)
    return A, S, V, T1, T2
...
sL = np.random.randint(0, p, 4)
sR = np.random.randint(0, p, 4)
t1 = random_element()
t2 = random_element()
